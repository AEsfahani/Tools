================================================================================
       Windows APPLICATION: SageExplorerBar Overview                        
===============================================================================
/////////////////////////////////////////////////////////////////////////////
Summary:
...
For 32bit IE on x86 or x64 OS, install SageExplorerBarSetup(x86).msi, the output
of the SageExplorerBarSetup(x86) setup project.

For 64bit IE on x64 OS, install SageExplorerBarSetup(x64).msi outputted by the 
SageExplorerBarSetup(x64) setup project.

B. Removal

For 32bit IE on x86 or x64 OS, uninstall SageExplorerBarSetup(x86).msi, the 
output of the SageExplorerBarSetup(x86) setup project. 

For 64bit IE on x64 OS, uninstall SageExplorerBarSetup(x64).msi, the output of
the SageExplorerBarSetup(x64) setup project.
...
Step1. Open this project in VS2010 and set the platform of the solution to x86. Make
       sure that the projects SageExplorerBar and SageExplorerBarSetup(x86)
	   ...
	   NOTE: If you want to run this sample in 64bit IE, set the platform to x64 and 
	         select SageExplorerBar and SageExplorerBarSetup(x64) to build.
        
Step2. Build the solution.
 
Step3. Right click the project SageExplorerBarSetup(x86) in Solution Explorer, 
       and choose "Install".
...
D. Deploying the IE Explorer Bar with a setup project.
  ...
  (1) In SageExplorerBar, add an Installer class (named IEExplorerBarInstaller 
  ...
  SageExplorerBarSetup(x86). Click OK to create the project. 
  ...
  In the Add Project Output Group dialog box, SageExplorerBar will  
  be displayed in the Project list. Select Primary Output and click OK. VS
  will detect the dependencies of the SageExplorerBar, including .NET
  ...
  SageExplorerBar. This adds the custom actions that we defined 
  in IEExplorerBarInstaller of SageExplorerBar. 
  ...
  must create a new setup project (e.g. SageExplorerBarSetup(x64) 
  ...
  file in the SageExplorerBarSetup(x64) project folder. To 
  configure the script to run in the post-build event, you select the 
  SageExplorerBarSetup(x64) project in Solution Explorer, and 
